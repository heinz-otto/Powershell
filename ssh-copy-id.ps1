...
#region Params
  ...
  (
    ...
    $ziel,$os = '',$i = "$env:userprofile\.ssh\id_rsa.pub"
  )
#endregion 
...
$cmd=@"
if ! grep -q `"`"$pkey`"`" .ssh/authorized_keys
then
  mkdir -p .ssh
...
"@
  ...
  $cmd="findstr /c:`"`"$pkey`"`" .ssh\authorized_keys||mkdir .ssh ||echo $pkey >>.ssh\authorized_keys"
}
